...
plot_model <- function(transcript_quantifier, gene_name = NULL,
  ...
  # Abundance track
  abund_track <- get_abund_track(tq, transcripts)
  if (!is.null.(abund_track)) {print("yes")}
  ...
  # Plot tracks
  args <- list(trackList = c(list(tx_track,
                                masks_track,
                                d_track_plus,
                                d_track_minus),
                             abund_track),
...
get_abund_track <- function(transcript_quantifier, transcripts){
  tq <- transcript_quantifier
  key <- tq@transcript_model_key
  target_group <- unique(key[key$tx_name %in% transcripts, ]$group)
  for (g in target_group) {
    target_models <- tq@models[[g]]
    target_abund <- tq@model_abundance[[g]]
    target_models_nrow <- nrow(tq@models[[g]])
    target_bins <- tq@bins[[g]]
    target_meta <- target_models * matrix(rep(target_abund, target_models_nrow),
                                            nrow = target_models_nrow,
                                            byrow = TRUE)
    dtrack <- c(dtrack, Gviz::DataTrack(range = target_bins,
                                        data = t(target_meta),
                                        groups = names(target_abund),
                                        type = "histogram"))
  }
  return(dtrack)
